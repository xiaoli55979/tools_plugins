...
class ListViewShowDataState extends State<ListViewShowData> {
  ...
  String getChannelAvatarURL(UIConversation uiConversation) {
    if (uiConversation.channelAvatar == '') {
      uiConversation.msg.getWkChannel().then((channel) {
        if (channel != null) {
          uiConversation.channelAvatar = "${HttpUtils.apiURL}/${channel.avatar}";
        }
      });
    }
  ...
  String getChannelName(UIConversation uiConversation) {
    ...
    print("object_channelName:${uiConversation.channelName}");
    ...
    return Container(
        ...
        child: Row(
          children: [
            Container(
              decoration:
                  const BoxDecoration(shape: BoxShape.rectangle, borderRadius: BorderRadius.all(Radius.circular(20)), color: Colors.blue),
              width: 50,
  ...
  longClick(UIConversation uiMsg, BuildContext context, LongPressStartDetails details) {
