...
class WKConnectionManager {
  ...
  Future<void> _webSocketConnect(String addr) async {
    Logs.info("尝试连接 WebSocket: $addr");
    final wsUrl = Uri.parse(addr);
    final channel = WebSocketChannel.connect(wsUrl);
    try {
      await channel.ready; // 确保连接成功
      _webSocket = channel;
      Logs.info("WebSocket 连接成功: $addr");
      _connectSuccess();
    } catch (e) {
  ...
  /// 判断是不是tck地址
  bool _isTcpAddr(String address) {
    // 正则表达式匹配 TCP 地址格式 (hostname:port 或 IP:port)
    final tcpAddrPattern = RegExp(
      r'^(?:[a-zA-Z0-9.-]+|\d{1,3}(\.\d{1,3}){3}):\d{1,5}$',
    );
    return tcpAddrPattern.hasMatch(address);
  }
  ...
  void _connectSuccess() {
    ...
    if (WKIM.shared.options.useWebSocket) {
      _webSocket?.stream.listen(
        ...
        onDone: () {
          Logs.debug("WebSocket onDone，Code: ${_webSocket?.closeCode}, Reason: ${_webSocket?.closeReason}");

          // Logs.debug("WebSocket 连接已关闭_isReconnection:$isReconnection");
          // setConnectionStatus(WKConnectStatus.fail);
          isDisconnection = true;
          // if (!isReconnection) {
          //   isReconnection = true;
          //   Future.delayed(Duration(milliseconds: reconnMilliseconds), () {
          //     Logs.debug("WebSocket 断开，尝试重新连接...");
          //     connect();
          //   });
          // }
        },
        onError: (error) {
          Logs.debug("WebSocket 发生错误: $error");
          isDisconnection = true;
          setConnectionStatus(WKConnectStatus.fail);
          if (isDisconnection) {
            Logs.debug("连接被手动断开，不重连");
            return;
          }

          // if (!isReconnection) {
          //   Future.delayed(Duration(milliseconds: reconnMilliseconds), () {
          //     Logs.debug("WebSocket 发生错误，尝试重新连接...");
          //     connect();
          //   });
          // }
        },
  ...
  _sendPacket(Packet packet) async {
    ...
    if (WKIM.shared.options.useWebSocket) {
      if (_webSocket != null && !isReconnection) {
    ...
    if (_sendingMsgMap.isNotEmpty) {
      ...
      while (it.moveNext()) {
        ...
        if (wkSendingMsg.sendCount == 5 && wkSendingMsg.isCanResend) {
          ...
          var nowTime = (DateTime.now().millisecondsSinceEpoch / 1000).truncate();
          if (nowTime - wkSendingMsg.sendTime > 10) {
            wkSendingMsg.sendTime = (DateTime.now().millisecondsSinceEpoch / 1000).truncate();
