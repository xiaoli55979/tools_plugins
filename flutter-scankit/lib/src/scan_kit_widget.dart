...
class ScanKitWidget extends StatelessWidget {
  ...
  ScanKitWidget({required this.controller, this.boundingBox, int format = scanTypeAll, bool? continuouslyScan})
      : this.format = format,
  ...
  Widget build(BuildContext context) {
    var map = {};
    if (boundingBox != null) {
      map["boundingBox"] = [boundingBox!.left.toInt(), boundingBox!.top.toInt(), boundingBox!.width.toInt(), boundingBox!.height.toInt()];
    }
...
class ScanKitController {
  ...
  final StreamController<ScanResult> _resultStreamController = StreamController();

  final StreamController<bool> _visibleStreamController = StreamController();
  ...
  ScanKitController() : _api = ScanKitApi() {
    _id.future.then((value) {
      _eventChannel = EventChannel('xyz.bczl.scankit/embedded/result/$value');
      _eventSubscription = _eventChannel!.receiveBroadcastStream().where((event) => event is Map).listen(_eventHandler, cancelOnError: false);
    });
  }
  ...
  Future<void> pickPhoto(BuildContext context) async {
    ...
    if (image != null) {
      var myData = await image.readAsBytes();
      ...
      var qrValue = result.originalValue;
      Future(() {
